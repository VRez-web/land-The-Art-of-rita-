const textSlides = (direction) => {
  ...
  if (nextSlide && !nextSlide.classList.contains("main-section__explore")) {
    ...
    tl.to(currentSlide.querySelector('.slides-container__title'),.6,{
        opacity:0,
        y:100
    })
    .to(currentSlide.querySelector('.designers-info'),.6,{
        opacity:0,
        y:100
    },'-=.6')
    .to(nextSlide.querySelector('.slides-container__title'),.6,{
        opacity:1,
        y:0
    },'-=.1')
    .to(nextSlide.querySelector('.designers-info'),.6,{
        opacity:1,
        y:0
    },'-=.5')
